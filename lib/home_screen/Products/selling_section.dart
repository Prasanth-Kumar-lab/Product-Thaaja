...
Widget buildBestSellingCard(Product product) {
  return BestSellingCard(product: product);
}
class BestSellingCard extends StatelessWidget {
  final Product product;
  const BestSellingCard({Key? key, required this.product}) : super(key: key);
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: 15),
                  Obx(() {
                    int count = cartController.getCount(product);
                    double totalPrice = count * product.price; // Calculate total price
                    return Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text(
                          count == 0
                              ? "\$${product.price.toStringAsFixed(2)}" // Default price when count is 0
              ...
              Positioned(
                ...
                child: GestureDetector(
                  onTap: () {
                    ...
                    favoriteController.toggleFavorite(product); // Toggle favorite
                  },
                  child: Obx(() {
                    return Icon(
                      Get.find<FavoriteController>().isFavorite(product)
                          ? Icons.favorite
